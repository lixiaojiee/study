Dubbo采用软负载均衡的方式处理服务的分发，负载均衡也是采用插件的形式实现，Dubbo中实现的负载均衡方式有四种，分别是随机、轮询、最小活跃数、一致性哈希。默认采用随机负载均衡，如果以上四种方式不满足需求，用户也可以自己实现。
...
**问题：** 对于`FailoverClusterInvoker`，在第一次失败后，重新选择第二个可调用服务，在负载均衡中计算权重的时候，如果因为临时修改权重导致负载均衡又选择了第一次调用失败的服务，这个时候怎么办？
...
Dubbo的一致性哈希负载均衡，通过在消费端的配置文件中配置hash.arguments指定对哪个方法的哪些参数进行哈希映射，具体的指定方式是指定参数索引，从0开始，若需要指定多个参数，则需要将各参数索引用逗号分隔，默认为第一个参数，即索引值为0。通过将指定的参数转换成哈希值来决定将该请求路由到哪台服务器上
...
digest是将指定的参数拼接后通过MessageDigest生成的一个md5数组，计算的时候是将每4位分一组，得到一个哈希值，md5数组如下：
